...
def main():
	# all20_LSI() # This function runs LSI on the data while sweeping across many dimensions
	all20_NMF() # This function runs NMF on the data while sweeping across many dimensions
	# all20_NMF_NLT() # This function runs NMF with a Log NLT on the data while sweeping across many dimensions
...
def all20_NMF_NLT():
	...
	workbook = xlsxwriter.Workbook('part5_All20_NMF_NLT.xlsx')

	for dims in nmf_nlt_dim_bank:

		print("Implementing NMF of dimension %d on data..." % dims)
		nmf_ = NMF(n_components=dims)
...
def all20_NMF():
	...
	# Reducing the dimensionality with NMF

	nmf_dim_bank = range(1,21)

	workbook = xlsxwriter.Workbook('part5_All20_NMF.xlsx')

	for dims in nmf_dim_bank:
...
def all20_LSI():
	...
	# Reducing the dimensionality with LSI

	workbook = xlsxwriter.Workbook('part5_All20_LSI.xlsx')

	lsi_dim_bank = range(1,21)

	for dims in lsi_dim_bank:

		print("Implementing LSI (TSVD) on data with dimension %d" % dims)
		lsitsvd = TSVD(n_components = dims) # What should this be?
